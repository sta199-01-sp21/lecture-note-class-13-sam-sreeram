...
The downside to using point estimates is that it is difficult to reach the 
actual value right off the bat.
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-sam-sreeram/manhattan.csv")
```
...
This tells us how confident we are that the interval will contain the true value. 
...
We are 95% confident that the true proportion of mice that died while not on the treatment is in the interval between 69% and 79.3%.
...
This indicates that we are 90% confident that the true proportion of mice that lived after being treated is between the interval from 55.2% to 65.1%.
